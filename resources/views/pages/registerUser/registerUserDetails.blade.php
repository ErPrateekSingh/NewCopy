...
<div class="container">
    <div class="row">
        <div class="col-md-10 col-md-offset-1 col-xs-12">
            ...
            @endif
            <div class="panel panel-form clearfix" style="border-color: #ccc;">
                <div class="panel-circle">
                  <i class="fa fa-user" style="margin-top: 8px;"></i>
                  <span style="line-height: 1.1;">User<br>Details</span>
                </div>
...
<script>
    ...
    function validateForm() {
        ...
        if (dob == '' || dob.indexOf(' ') >= 0) {
            $("#dob").val("");
            valid = hasError('dob', 'Date of Birth is required!');
        } else {
            if (dob.length !== 10) valid = hasError('dob', 'Invalid Date format!');
            else hasSuccess('dob');
        }
        if ($("#gender").val() == '' || $("#gender").val() == null) valid = hasError('gender', 'Gender is required!');
        else hasSuccess('gender');
        if ($("#state").val() == '' || $("#state").val() == null) valid = hasError('state', 'State is required!');
        else hasSuccess('state');
        if ($("#city").val() == '' || $("#city").val() == null) valid = hasError('city', 'City is required!');
        else hasSuccess('city');
        return valid;
    }
    $(document).ready(function() {
        ...
        $('#username').on('keyup', function() {
            ...
            if (!username.match(/^[a-zA-Z0-9]+$/)) {
                field_error("username", "Only Alphabets and Numbers are allowed!");
                usernameValid = false;
            } else if (username.length >= 6) {
                if (username.length >= 20) {
                    field_error("username", "Maximum 20 characters allowed!");
                    usernameValid = false;
                } else {
                    delay(function() {
        ...
        $('#registerUserDetails').on('submit', function(e) {
